...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance=0mm, auto, font=\tiny]
		\node[draw=none, rectangle, minimum height=0.05cm, minimum width=7.5cm, fill=black!75, inner sep=0pt]
			(timeline) {};
		\draw[{Circle[length=0.75mm]}-{Circle[length=0.75mm]}, NavyBlue]
			([yshift=2mm]timeline.north west) -- ++(1.25cm,0cm);
		\foreach \x in {1.25, 2.5, 3.75, 5.00, 6.25}{
			\draw[-{Circle[length=0.75mm]}, NavyBlue] ([yshift=2mm, xshift=\x cm]timeline.north west) -- ++(1.25cm,0cm);
		}
		\draw[{Circle[length=0.75mm]}-{Circle[length=0.75mm]}, NavyBlue]
			([yshift=4mm, xshift=0cm]timeline.north west) -- ++(3.75cm,0cm);
		\draw[-{Circle[length=0.75mm]}, NavyBlue] ([yshift=4mm, xshift=3.75cm]timeline.north west) -- ++(3.75cm,0cm);
