...
\subsubsectionboxEDIT{Ordering or Setup Cost: $\boldsymbol{c_t} \to \nicefrac{\$}{\text{order}}$}{}{
...
\breakspace
\textbf{Administrative Costs:} Communication costs, procurement software/systems (usage, maintenance, training),
paperwork, invoicing, etc.
When a buyer is engaged in the ordering process, their time investment translates into a direct cost.
However, theoretically, this cost is negligible if the buyer is idle. 
\breakspace
...
the lot. For instance, handling and transporting large quantities incurs more costs than a smaller lots.
This implies that these costs comprise both a fixed element and a variable element that escalates with the lot 
size. The resulting setup cost would look like:

\[c_t = c_{t_v}Q + c_{t_f}\]

Notice that the $Q$ in the variable component would be cancelled out when multiplying by $\nicefrac{D}{Q}$. 
This is why costs like transportation and material handling were already included in the unit cost $c$. However,
this is assumes that the factor $c_{t_v}$ is unique throughout the year.

\breakspace

These costs could be even more complex, potentially adopting a piecewise function approach. For instance, 
surpassing a certain quantity threshold might require an additional vehicle or labor, thereby also augmenting the
fixed cost component, creating a scenario in which the whole setup cost is variable.

\[ c_t = \begin{cases}
	c_{t_v}Q + K_1) 	&,\quad \text{for} \; Q \leq \text{threshold}\\
	c_{t_v}Q + K_2) 	&,\quad \text{for} \; Q > \text{threshold}
\end{cases} \]

\textbf{FIXED COSTS OF MANUFACTURING:}
Similar to the buyer case, here we could have a 
machinist that is in charge of setting machines up, and could also be busy or idle. There's a setup learning
period in which a new production setup could produce items at a slower pace or low quality, so these factors should 
also be included. Opportunity Costs: If a factory is running at full capacity, every minute of production time is
valuable because it could be use to produce items that already generate revenue.
When the production line is halted to set up for another item, the company loses the opportunity to
produce and sell goods during that time, resulting in a loss of potential revenue. 
This is the opportunity cost of setup time.
...
\[ \text{Total Holding Cost} = \begin{cases}
	Q(rc + h) 						&,\quad \text{for} \; Q \leq \text{threshold}\\
	Q(rc + h) + \text{Fixed Cost} 	&,\quad \text{for} \; Q > \text{threshold}
\end{cases} \]
