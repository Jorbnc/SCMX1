...
\subsubsectionboxEDIT{Ordering/Setup Cost: $\boldsymbol{c_t} \to \nicefrac{\$}{\text{order}}$}{}{
\Red{Refine this with GPT-:}
\breakspace
Also known as ordering cost. Represents a fixed cost incurred each time a batch is produced or a lot is purchased.
The main idea is that this cost is independent of the lot size. Let's say our lot size is Q. That is, we will 
always replenish a quantity Q of inventory. Then, throughout the unit time we'll have to replenish D/Q times.
Thefore, the total setup cost will be the individual setup cost times D/Q.
\breakspace
Let's explore what could be the components of the individual setup cost:
\breakspace
When purchasing inventory: Fixed administrative costs: internal processes, paperwork, invoicing.
If the buyer is busy, then their time required to place the order has to be included as a cost.
If the buyer is idle, theoretically, no cost is incurred. We also have a fixed cost of receiving, composed of 
material handling and inspection costs, and the cost of updating inventory
records. In realisic scenarios, however, this component is rather lot-size-dependent.
Inspecting 100 items is more costly than inspecting 10 items. We can say the same for the transportation that 
the purchasing requires. Transportation could for instance have a fixed component and
a variable component that is lot-size-dependent. It's cost function may even be defined as a piecewise function, 
that is, if Q is greater than some threshold, another vehicle could be required, increasing the fixed component.
For simplicity, let's ignore the piecewise behaviour and consider only that there's a variable component.
 
 $c_t = c_{t_v}Q + c_{t_f}$
 
Notice that [...] this is why transportation and material handling costs were already included in $c$
\breakspace
What about the setup cost in a manufacturing context? Similar to the buyer case, here we could have a 
machinist that is in charge of setting machines up, and could also be busy or idle. There's a setup learning
period in which a new production setup could produce items at a slower pace or low quality, so this cost should 
