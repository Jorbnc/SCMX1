...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance=0mm, auto, font=\tiny]
		\node[draw=none, rectangle, minimum height=0.075cm, minimum width=7.5cm, fill=black!75, inner sep=0pt]
			(timeline) {};
		\draw[{Circle[length=0.8mm]}-{Circle[length=0.8mm]}, NavyBlue]
			([yshift=5mm]timeline.north west) -- ++(2.5cm,0cm);
		\foreach \x in {2.5, 5.00}{
			\draw[-{Circle[length=0.8mm]}, NavyBlue] ([yshift=5mm, xshift=\x cm]timeline.north west) -- ++(2.5cm,0cm);
		}

		\node[draw=none, inner sep=0pt, above, yshift=1.5mm] (i00) at (timeline.north west) {$I_o$};
		\node[draw=none, inner sep=0pt, above, xshift=2.5cm, yshift=1.5mm] (i00) at (timeline.north west) {$I_o$};
		...
		\foreach \x in {
			...
			\draw[-, line width=0.05mm, Green] ([xshift=\x cm, yshift=-0.75mm]timeline.south west) -- ++(0mm,-4mm);
		}
	\end{tikzpicture}
\end{figure}%
\vspace{-1mm}
