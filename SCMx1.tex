...
\sectionbox{Financial and Economic Analysis}
...
\begin{align*}
	...
	WACC = \frac{E}{E+D}(R_E + \beta\cdot MRP) + \frac{D}{E+D}R_D(1-t)
\end{align*}
Let's analyze this concept. First, notice the terms $\displaystyle\frac{E}{D+E}$ and $\displaystyle\frac{D}{E+D}$;
they create a weighted measure of the individual contributions of $(R_f + \beta\cdot MRP)$ and $R_b(1-t)$.
}

\subsection{Economic and Investment Analysis}

\subsubsectionbox{Time Value of Money}{}{
Foo
}

\subsubsectionbox{Present Value}{}{
Foo
}

\subsubsectionbox{Combine with Pratt 6?}{}{
Check if Pratt chapter 6 has something to do with Cash Flow Analysis (it seems, but haven't done a diligent search).
}

\subsubsectionbox{Free Cash Flow}{}{
Foo
}
...
\subsection{Units to include}

\subsubsectionboxEDIT{General Rule}{pratt2010financial}{
Items intended for manufacturing, sale, or consumption should be included in a company's 
...
\subsection{Costs to attach}

\subsubsectionboxEDIT{General Rule}{pratt2010financial, GPT4}{
Attach all the costs required to bring inventory to a saleable condition:
\begin{itemize}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=0mm, auto, font=\tiny]
		...
		\node[align=left] () at (3.75, 1.6) {$COGS_{t} = I_{t-1}+ P_{t} - I_{t}$};
...
\subsubsectionbox{FIFO vs. LIFO}{pratt2010financial}{
\vspace{-1mm}
\begin{figure}[H]
	\centering
	...
	\begin{tikzpicture}[
		...
		\node[txt, above=1.20cm of 12.south] () {\textbf{Beginning inventory}\\(layer n-2)};
		\node[txt, above=1.20cm of 25.south] () {\textbf{First Purchase}\\(layer n-1)};
		\node[txt, above=1.20cm of 33.south] () {\textbf{Second Purchase}\\(layer n)};

		\node[txt, left=0.45cm of 11.north west, anchor=east] () {\textbf{FIFO:}};

		% ===========================================================================================
		\node[Rect1, below=1.5cm of 11.south, anchor=north, fill=white] (l11) {};
		...
		\path (31.south west) -- (l31.north west)
			node[midway, yshift=1.25mm, xshift=-0.75mm, anchor=center, inner sep=0.2pt] 
			(salesthreshold) {1st, 2nd sale threshold};

		\draw[-, dash pattern=on1off1, line width=0.05mm] (salesthreshold.north) -- ++(0mm, 3.75mm);
		\draw[-, dash pattern=on1off1, line width=0.05mm] (salesthreshold.south) -- ++(0mm,-7mm);

	\end{tikzpicture}
\end{figure}
}

\subsubsectionbox{Weighted Average Method}{}{
...
\subsection{More on the LIFO Method}

\subsubsectionbox{The Dollar Value LIFO Method}{}{
Foo\\
$ \displaystyle\frac{\text{Total Inventory Cost}_{t}}{\text{Total Items}_{t}} = c_{t} $\\
$\text{Base Year Cost}_{t} = \displaystyle\frac{c_{t}}{c_{t-1}}$
}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.75, every node/.style={transform shape}, node distance=0mm, auto, font=\tiny]
		...
		\def\bcolor{Green!55}
		\tikzstyle{Rect1}=[draw, line width=0.05mm, rectangle, inner sep=0pt, align=left, fill=white];
		\tikzstyle{Rect2}=[draw, line width=0.05mm, rectangle, inner sep=0pt, fill=white];

		\node[Rect1, minimum width=2.00cm, minimum height=0.50cm] (c1) {smallest COGS\\layer};
...
\cite{kinney2004jit} observed the following behaviors related to Just-In-Time inventory management, LIFO reserve dynamics, 
and financial decision-making in firms during the 1980s and 1990s, a period in which JIT gained popularity:
\breakspace
\textbf{Large LIFO Reserve firms:} Firms with significant LIFO reserves often see important short-term earnings 
increases after JIT adoption, as the liquidation of lower-cost LIFO inventories reduces COGS and boosts profits.
\breakspace
...
\breakspace
\textbf{For merchants}:
The unit cost encompasses the purchase price from suppliers, plus any additional costs necessary for preparing 
the product for sale, such as packaging and labeling. It often also covers per-unit charges for freight 
transportation and material handling activities, like loading and unloading.
\breakspace
\textbf{For producers}:
The unit cost extends beyond the mere calculation of raw materials. It encompasses the comprehensive unitary 
production cost, which includes direct labor and overheads.
...
\subsubsectionbox{Ordering or Setup Cost: $\boldsymbol{c_t} \to \nicefrac{\$}{\text{order}}$}{
	2008building, silver2016inventory, chopra2016scm, cachon2018matching,  
}{

It's a fixed expense incurred with each batch production or lot purchase. 
Its defining characteristic in modeling is that it remains constant, irrespective of the lot size $Q$.
Accordingly, the total setup cost is solely based on the number of replenishments throught the unit time:

\[ \text{Total Setup Cost} = c_t\frac{D}{Q} \]

\textbf{FIXED COSTS OF PURCHASING:}
\breakspace
\textbf{Administrative Costs:} Buyer's time, communication costs, procurement software (usage, maintenance, training),
paperwork, invoicing.
\breakspace
\textbf{Receiving Costs:} Material handling, storage preparation, inspection, update of inventory records.
\breakspace
\textbf{Transportation Costs:} Fixed salaries or rates, maintenance and depreciation, fuel.
\breakspace
In practical situations, however, both receiving and transportation expenses are influenced by the size of 
the lot. For instance, handling and transporting large quantities incurs more costs than smaller lots.
This implies that these costs comprise both a fixed element and a variable element that escalates with the lot 
size. The resulting setup cost would look something like:

\[c_t = c_{t_{v}}Q + c_{t_f}\]

Notice that the $Q$ in the variable component would be cancelled out when multiplying by $\nicefrac{D}{Q}$. 
This is why costs like transportation and material handling were already included in the unit cost $c$.
However, this assumes that the factor $c_{t_{v}}$​ does not benefit from economies of scale and remains constant 
over the year.

\breakspace

The complexity of ordering costs may increase, potentially requiring a piecewise function model.
When certain quantity thresholds are exceeded, additional resources such as extra vehicles or labor might be 
needed. This introduces step-fixed costs, which remain constant within specific ranges but increase at these 
thresholds. Let $K_i$ be the fixed cost component for a range, then:

\[ c_t = \begin{cases}
	c_{t_v}Q + K_1 	&,\quad \text{for} \; Q < \text{threshold}\\
	c_{t_v}Q + K_2 	&,\quad \text{for} \; Q \geq \text{threshold}
\end{cases} \]
}

\subsubsectionbox{Manufacturing Setup Costs}{silver2016inventory}{
Similar to the buyer scenario in purchasing, the setup cost in manufacturing also involves the machinist's time 
for machine setup. This cost varies significantly based on the machinist's availability – higher if they are 
diverted from other busy tasks, and lower if they are otherwise idle.
\breakspace
During the setup learning period, a new production setup may initially lead to slower production rates or lower 
quality outputs. This phase not only affects direct production efficiency but also incurs additional costs 
related to training and heightened supervision.
\breakspace
Opportunity Costs play a critical role: When a factory running at full capacity pauses production for setup, 
it loses potential revenue that could be generated from regular operations. This loss extends beyond immediate 
financial impact to potential delays in order fulfillment and potential long-term effects on customer 
satisfaction, underlining the broader implications of setup time as an opportunity cost.
}
...
\breakspace
\textbf{Insurance Premiums}, which safeguard against theft or damage, and \textbf{Taxation Expenses}, based on 
the inventory's value and location, may also be included.
}
...
\[ \text{Total Holding Cost} = \begin{cases}
	Q(rc + h) 						&,\quad \text{for} \; Q < \text{threshold}\\
	Q(rc + h) + \text{Fixed Cost} 	&,\quad \text{for} \; Q \geq \text{threshold}
\end{cases} \]
...
\[ TC = Total\;Material\;Cost + Total\;Setup\;Cost + Total\;Holding\;Cost + Total\;Shortage\;Cost \]

\[ TC = cD + c_{t}\frac{D}{Q} + c_{e}\frac{Q}{2} + c_{s}E[Units\;Short] \]
...
\begin{align*}
	...
	\shortintertext{The $EOQ$ or $Q^*$ gives the minimum $TRC$ under deterministic conditions:}
\end{align*}%
\vspace{-0.80cm}
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{axis}[1Quad, width=9cm, height=3.25cm, ymin=0, ymax=1.3, xmin=0, xmax=\lastx + 0.25,
			...
			(1,0) -- (2,0) node [annot_color,midway,yshift=-12.5pt, line width=5mm] {\tiny $\displaystyle T^*=\frac{Q^*}{D}$};
			\draw (0,1) node[annot_color, left]{\tiny $Q^*$};
			\draw (0,0.5) node[annot_color, left]{\tiny $\displaystyle\frac{Q^*}{2}$};
