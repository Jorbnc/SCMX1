...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1mm, auto, font=\tiny]
		...
		\draw[-{Stealth[length=2mm]}] ([yshift=0.65cm]rect2.south west) -- 
		...
		\draw[-{Stealth[length=2mm]}] ([yshift=0.25cm]rect1.south east) --
			node[above, yshift=-0.5mm, align=center, font=\tiny]
				{Capital}
			([yshift=0.25cm]rect2.south west);

		\draw[-{Stealth[length=2mm]}] ([xshift=4mm]rect2.south west) --
			++(0,-0.525cm) -| node[pos=0.25, above, yshift=-0.2mm, align=left, font=\tiny]
				{Debt payments\\
			 	Dividends, Stock buybacks}
			 (rect1.south);
		\draw[-{Stealth[length=2mm]}] ([xshift=7mm]rect2.south west) --
			++(0,-0.525cm) -| node[pos=0.25, above, yshift=-0.2mm, align=left, font=\tiny]
				{Reinvestment}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=0.6mm, auto, font=\tiny]
		...
		\tikzstyle{posRect}=[draw, rectangle, minimum height=0.25cm, fill=GreenFigA, inner sep=0pt];
		\tikzstyle{negRect}=[draw, rectangle, minimum width=0.1cm, minimum height=0.25cm, fill=RedFigA, inner sep=0pt];
		\tikzstyle{textRect}=[draw=none, rectangle, minimum height=0.25cm, align=right, inner sep=0pt];
...
Create a flowchart (rather than a cycle figure) for this:
\begin{itemize}
	\item Acquisition: Purchase or Manufacture
	\item Carrying Inventory: Periodic or Perpetual Methods
	\item Selling Invetory: FIFO, LIFO and register COGS for Income Statement
	\item or goes to ---> Ending Inventory
\end{itemize}
}
...
\subsubsectionbox{Units to include in Inventory}{}{
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance=1mm, auto, font=\tiny]
		\node[draw=red, minimum width=2cm, minimum height=1cm] (rect1) {$123$};
	\end{tikzpicture}
\end{figure}
}

\subsubsectionbox{Consignments}{}{In a consignment arrangement, the \textit{consignor} transfers inventory to a
...
proceeds to the consignor. Inventory is only included in the consignor's balance sheet.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1mm, auto, font=\tiny]
		...
		\node[Rect, right=1.5cm of consignor.east, anchor=west] (consignee) {Consignee};
		\node[Rect, right=1.5cm of consignee.east, anchor=west] (customer) {Customer};
		\draw[-{Stealth[length=1mm]}] ([yshift=0.8mm]consignor.east) -- ([yshift=0.8mm]consignee.west);
		\draw[-{Stealth[length=1mm]}] ([yshift=0.8mm]consignee.east) -- ([yshift=0.8mm]customer.west);
		\draw[-{Stealth[length=1mm]}, green!70!black] ([yshift=-0.8mm]customer.west) -- ([yshift=-0.8mm]consignee.east);
		\draw[-{Stealth[length=1mm]}, green!70!black] ([yshift=-0.8mm]consignee.west) -- ([yshift=-0.8mm]consignor.east);
	\end{tikzpicture}
\end{figure}
}

\subsubsectionbox{Goods in Transit}{}{
...
\subsubsectionbox{Costs to Attach}{}{
Foo
}

\subsubsectionbox{Record of carried inventory: Periodic and Perpetual Methods}{}{
Foo
}

\subsubsectionbox{COGS: FIFO vs. LIFO}{}{
Foo
}
...
\begin{minipage}{0.6\linewidth}\centering
	\begin{figure}[H]
		...
		\begin{tikzpicture}[auto, font=\tiny]
			...
			\draw[->, NavyBlue] (0,0) [partial ellipse=25:-23:2cm and 1cm];
...
\begin{center}
Companies, Investors, and Flow of Capital
